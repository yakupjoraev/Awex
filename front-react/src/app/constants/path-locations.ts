export const HOME_PAGE_PATH = "/"
export const ASSETS_ROUTE = "/assets"
export const NOTIFICATIONS_PAGE = "/notifications"

export const ADMIN_MERCHANTS_ROUTE = "/admin/merchants"
export const ADMIN_MERCHANT_STATS_SUBROUTE = "/stats"
export const ADMIN_STATS_ROUTE = "/admin/statistics"
export const ADMIN_APPLICATIONS_ROUTE = "/admin/applications"
export const ADMIN_APPLICATIONS_PROJECTS_ROUTE = "/admin/applications/projects"
export const ADMIN_APPLICATIONS_PROJECTS_DETAILS_ROUTE = "/admin/applications/projects"
export const ADMIN_COMMISSION = "/admin/commission"

export const ROUTE = {
  HOME_PAGE_PATH: '/',
  INVOICE_PATH: '/invoice',
  INVOICE_TEMPLATES_PATH: '/invoice-templates',
  SUCCESSFULLY_INVOICE_PATH: '/successfully-invoice',
  DEPOSITS_PATH: '/deposits',
  DEPOSIT_RETENTION_PATH: '/deposit-retention',
  PROJECTS_PATH: '/projects',
  PROJECTS_PROJECTID_PATH: '/projects',
  PROJECTS_NEW_PROJECT_PATH: '/projects/new-project',
  SETTINGS_PATH: '/settings',
  ASSETS_ROUTE_PATH: '/assets',
  ASSETS_ROUTE_ASSETID_ACTION_PATH: '/assets',
  INFOCENTER_PATH: '/infocenter',
  DATE_PICKER_PATH: '/date-picker',
  NOTIFICATIONS_PATH: '/notifications',
  HISTORY_PATH: '/history',
  REFERRAL_PATH: '/referral',
  PAYMENT_CRYPTO_STAGE_PATH: '/payment-crypto',
  PAYMENT_UNIQUEID_PATH: '/payment',
  INDEX_PATH: '/index',
  AUTH_PATH: '/auth',
  ADMIN_AUTH_PATH: '/admin/auth',
  INTEGRATION_PATH: '/integration',
  INTEGRATION_API_PATH: '/integration-api',
  INTEGRATION_CMS_MODULE_PATH: '/integration-cms-module',
  INTEGRATION_HTML_WIDJET_PATH: '/integration-html-widjet',
}